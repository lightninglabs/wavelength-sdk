...
Direct clients also receive a terminal `activityStream` event when a stream
ends or fails. The client does not retry automatically, so a direct consumer
chooses its own retry policy. The current bridges do not provide structured gap
cursor or reason details.
