...
description: How WalletDK represents spendable funds as Virtual Transaction Outputs (VTXOs) and tracks their lifecycle from boarding through Ark rounds to final confirmation.
---
...
WalletDK tracks spendable funds the way Ark does: as off-chain outputs bundled into the operator's periodic batch transactions, not as a single on-chain UTXO in your app. The sections below walk through that model, how boarding turns into live balance, and why confirmed and pending numbers can differ.
...
A VTXO behaves like a UTXO you can spend, but it lives in Ark's shared batch structure until you exit, refresh, or the operator sweeps expired batches. WalletDK tracks every live VTXO the daemon knows about and uses them as inputs when you send, swap, or leave.
...
VTXOs also have a **sweep delay** tied to their batch. Before the operator becomes eligible to sweep the batch, the VTXO should either be refreshed through a **batch swap** (getting a fresh VTXO in a newer batch) or moved to on-chain Bitcoin. This refresh is daemon-driven: WalletDK detects the approaching expiry and submits the batch swap for you, so there is no method to call yourself. WalletDK surfaces expiry pressure through balance and activity updates rather than asking you to manage raw outpoints manually.
...
When you show balance in a UI, users usually want one "available" number. WalletDK splits the picture into confirmed and pending fields so you can explain why spendable funds lag behind what they see on-chain or in the activity feed.
...
Every VTXO you receive this way inherits the batch's **CSV expiry** (sweep delay). As that horizon approaches, the VTXO is **refreshed** with a **batch swap**: forfeiting the old VTXO in a round in exchange for a fresh one in a newer batch, which resets the expiry clock. WalletDK handles the round interaction; you do not pick batch transaction IDs yourself. See [Leaving Ark](/concepts/leaving-ark/) for the `exit()` / `getExitPlan()` / `sweepWallet()` APIs that apply if you choose to move funds on-chain instead of refreshing.
