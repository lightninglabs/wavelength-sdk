...
start unilateral unroll. Once a unilateral job exists, follow-up
`exitStatus({ outpoint })` and
`sweepWallet({ destinationAddress, broadcast: false })` calls drive the
unroll to completion. Call the sweep with `broadcast: false` first to preview.
