...
to the uncompressed `wavewalletdk.wasm` if it cannot be used. Host both files so
...
inflated, and one labelled `application/gzip` all load the same way, and
compilation streams in every case. Compress it however your host makes easiest.
