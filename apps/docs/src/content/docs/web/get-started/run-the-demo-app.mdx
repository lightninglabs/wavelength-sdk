...
```bash title="Terminal"
# Expects a sibling wavelength checkout (override with WAVELENGTH_DIR)
pnpm --filter web-wallet-demo run wasm:local
```

This runs `make wasm-wallet` in wavelength and copies the full
...
chain, run [arktest](https://github.com/lightninglabs/wavelength) in a
separate terminal:

```bash title="Terminal"
# From a wavelength checkout
make build
