...
description: Solutions to common WalletDK integration errors and guidance for diagnosing stuck or pending sends.
---
...
The client throws `WalletDKError` with code **`asset_load_failed`** when
`walletdk.wasm.gz`, `wasm_exec.js`, or the SQLite bridge cannot be fetched.
...
`usePasskeyWallet(...).supported` stays `false`, or ceremonies fail with
...
On the web, `usePasskeyWallet` (from `walletdk-react`) takes a `PasskeyCeremony`
implementation as its argument; pass `webPasskeyCeremony` from
`@lightninglabs/walletdk-web`. See the [passkey guide](/guides/use-a-passkey/)
...
shell). For **regtest**, start arktest and match the ports in your
`RuntimeConfig` to what the environment prints (`./arktest env` or the
`frontend-regtest` overlay). URLs must include the scheme (`http://` or
`https://`). The demo's regtest defaults assume Ark on `7071`, Esplora on
`8501`, and the swap REST gateway on `10032` (these are the regtest defaults
returned by `defaultConfig('regtest')`).
...
to a specific `WalletDKError` code, so a persistent 401 surfaces client-side
(if at all) as a generic `walletdk_error`, not a dedicated code you can match
on.
...
After upgrading `@lightninglabs/walletdk-web` or rebuilding WASM, the browser
...
Lightning sends are **out-swaps** through the swap server, not direct channel
...
1. Call **`refresh()`** (from `useWalletDK()` or `useWalletRuntime()` in
   `walletdk-react`) or rely on the provider's activity subscription (the
   React provider debounces activity-driven refreshes). Without React, call
