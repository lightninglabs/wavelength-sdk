...
[`useWalletPasskey(ceremony)`](/reference/walletdk-react/#useWalletPasskey) drives the passkey ceremony and opens the wallet
...
Pass [`webPasskeyCeremony`](/reference/walletdk-web/#webPasskeyCeremony) from `@lightninglabs/walletdk-web` as the ceremony
implementation.
...
Pass [`createNativePasskeyCeremony({ rpId })`](/reference/walletdk-react-native/#createNativePasskeyCeremony) from
...
The underlying capability probe ([`ceremony.supportsPasskeyPrf()`](/reference/walletdk-web/#supportsPasskeyPrf)) is
memoized: only the first call per ceremony instance actually runs it, and
every [`useWalletPasskey`](/reference/walletdk-react/#useWalletPasskey) call sharing that instance resolves from the same
...
```tsx title="passkey.tsx"
import { webPasskeyCeremony } from '@lightninglabs/walletdk-web';
import { useWallet, useWalletPasskey } from '@lightninglabs/walletdk-react';
...
[`PasskeyCancelledError`](/reference/walletdk-core/#PasskeyCancelledError) instead of a regular failure, and that rejection is
...
```tsx title="passkey.tsx"
import { webPasskeyCeremony } from '@lightninglabs/walletdk-web';
import { PasskeyCancelledError, useWalletPasskey } from '@lightninglabs/walletdk-react';
...
file all surface as [`PasskeyCancelledError`](/reference/walletdk-core/#PasskeyCancelledError) here, indistinguishable from the
...
with a genuine error (not a [`PasskeyCancelledError`](/reference/walletdk-core/#PasskeyCancelledError)). Catch it and offer a
...
restore on a new device without a passkey, call `create({ password,
mnemonic })` from [`useWalletCreate()`](/reference/walletdk-react/#useWalletCreate) with a new wallet password and the
saved recovery phrase; `password` is required by [`CreateWalletRequest`](/reference/walletdk-core/#createWallet). This
...
function UnlockWithFallback({ credentialId }: { credentialId: string }) {
  ...
  if (recovering) {
    return (
      <form onSubmit={e => {
        e.preventDefault();
        create({ password, mnemonic: mnemonic.trim().split(/\s+/) });
      }}>
        <input
          type="password"
          value={password}
