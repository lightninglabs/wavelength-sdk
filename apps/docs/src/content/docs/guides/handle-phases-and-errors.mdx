...
`phase` from [`useWallet()`](/reference/walletdk-react/#useWallet) describes where the wallet is in its lifecycle.
...
| `restoring` | A background restore ([`useWalletRestore()`](/reference/walletdk-react/#useWalletRestore)) is bringing a freshly restored wallet up; setting `recoverState: true` on the request additionally tracks the recovery scan through the recovery state. |
...
`info.walletState` (from [`useWalletInfo()`](/reference/walletdk-react/#useWalletInfo)) carries the underlying
[`WalletState`](/reference/walletdk-core/#WalletState) string (`none`, `locked`, `ready`, or `syncing`) when you need
the daemon-level detail instead of the broader [`RuntimePhase`](/reference/walletdk-core/#RuntimePhase).
...
from [`useWallet()`](/reference/walletdk-react/#useWallet) for the reason.

## The Error | null convention

Every mutation hook ([`useWalletCreate`](/reference/walletdk-react/#useWalletCreate), [`useWalletRestore`](/reference/walletdk-react/#useWalletRestore), [`useWalletUnlock`](/reference/walletdk-react/#useWalletUnlock),
[`useWalletDeposit`](/reference/walletdk-react/#useWalletDeposit), [`useWalletReceive`](/reference/walletdk-react/#useWalletReceive), [`useWalletPrepareSend`](/reference/walletdk-react/#useWalletPrepareSend),
[`useWalletSend`](/reference/walletdk-react/#useWalletSend), [`useWalletRefresh`](/reference/walletdk-react/#useWalletRefresh), and the `create`/`open` pair on
[`useWalletPasskey`](/reference/walletdk-react/#useWalletPasskey)) exposes the same throw-and-capture shape: an action
function, a verb-prefixed `<verb>Pending: boolean`, a verb-prefixed
`<verb>Error: Error | null`, and (except [`useWalletRefresh`](/reference/walletdk-react/#useWalletRefresh) and
[`useWalletPasskey`](/reference/walletdk-react/#useWalletPasskey), neither of which exposes a `<verb>Data` field) a
...
`error` at the top level, from [`useWallet()`](/reference/walletdk-react/#useWallet), is the same `Error | null`
...
Every WalletDK-originated error extends [`WalletDKError`](/reference/walletdk-core/#WalletDKError) and carries a `code`
