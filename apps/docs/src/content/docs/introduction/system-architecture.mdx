...
Once started, the daemon reaches three external backend services. Configure them in `RuntimeConfig` (or use your transport package's `defaultConfig(network)` for the public signet, testnet, and testnet4 presets):
