...
<PackageList packages={[
  { name: '@lightninglabs/wavelength-core', role: 'Shared types, the WavelengthClient interface, errors, and enums. No DOM, no transport.' },
  { name: '@lightninglabs/wavelength-web', role: 'The browser WASM transport. Framework-agnostic: use it from vanilla JS, Vue, Svelte, or React. Re-exports core.' },
  { name: '@lightninglabs/wavelength-react-native', role: 'The React Native transport. The daemon is compiled into the app binary. Re-exports core.' },
  { name: '@lightninglabs/wavelength-react', role: 'React provider and hooks. Depends on core only and takes an injected client, so the same binding can run over the web or React Native transport.' },
]} />
