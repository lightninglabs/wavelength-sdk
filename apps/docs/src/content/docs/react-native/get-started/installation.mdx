...
are not included in the npm package. Stage them into the package (`android/libs/`
and `ios/` inside `@lightninglabs/wavelength-react-native`) before running
`pod install` or a Gradle build. Either download them from the paired
[wavelength release](https://github.com/lightninglabs/wavelength/releases) and
copy them to those paths yourself, or run the package's
`scripts/fetch-bindings.sh` against a `wavelength` checkout, which builds and
stages them for you.
</Callout>
