...
BOLT 11 is the standard Lightning invoice format. Every Lightning payment WalletDK sends or receives is a BOLT 11 invoice, so the wallet interoperates with any Lightning wallet, exchange, or app. See the [Lightning receive](#lightning-receive) and [Lightning send](#lightning-send) entries for how Lightning sends and receives are handled as swaps.
