...
| Flag | Default | Description |
|---|---|---|
| `--outpoint` | (none) | VTXO outpoint(s) to refresh (txid:index); repeatable. Required unless --all is set. |
...
```bash title="Terminal"
darepocli ark vtxos leave --outpoint abcd...:0 --address bcrt1...
```
...
```bash title="Terminal"
darepocli ark rounds get --round-id r-1234
```
...
```bash title="Terminal"
darepocli ark oor get --session-id s-1234
```
...
```bash title="Terminal"
darepocli ark send inround --to bcrt1... --amount 10000
```
