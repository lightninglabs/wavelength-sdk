...
configured with four persistent flags, available on every command:
...
A local regtest daemon typically runs without TLS on the default address, so
disable TLS and keep the default server. Export the connection flags once, for
example via a shell alias, so you do not repeat them on every call.
</Callout>
