...
- **The SDK** ([What is Wavelength?](/introduction/what-is-wavelength/)) compiles `waved` to WebAssembly and runs it **inside the browser**. Each of your users gets their own wallet instance, with keys held on their device. There is no server-side daemon to run or operate.
...
The API surface is versioned together with the daemon, not independently. It is currently **pre-release**: method signatures and JSON field names can change between `wavelength` releases. Pin a specific daemon version for production integrations.
