#!/usr/bin/env bash
# Build the walletdk gomobile bindings from the darepo-client checkout and
# stage them into this package: android/libs/Walletdk.aar and
# ios/Walletdk.xcframework. Both are gitignored; run this after cloning and
# again whenever the paired daemon revision changes (the same revision that
...
BUILD="$DAREPO/sdk/walletdk/mobile/build"

if [[ "$TARGET" == "android" || "$TARGET" == "all" ]]; then
  mkdir -p "$PKG/android/libs"
  rm -f "$PKG/android/libs/Walletdk.aar"
  cp "$BUILD/android/Walletdk.aar" "$PKG/android/libs/"
  echo "Staged android/libs/Walletdk.aar"
fi

if [[ "$TARGET" == "ios" || "$TARGET" == "all" ]]; then
  mkdir -p "$PKG/ios"
  rm -rf "$PKG/ios/Walletdk.xcframework"
  cp -R "$BUILD/ios/Walletdk.xcframework" "$PKG/ios/"
  ...
  find "$PKG/ios/Walletdk.xcframework" -name '*.h' \
    -exec sed -i '' 's|@import Foundation;|#import <Foundation/Foundation.h>|' {} +
  echo "Staged ios/Walletdk.xcframework"
fi
